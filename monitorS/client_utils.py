...
class TrayIcon:
    ...
    def create_icon(self):
        ...
        # ===== 修改菜单，添加"重新配置"选项 =====
        menu = (
            self.pystray.MenuItem("📊 显示状态", self.show_status),
            self.pystray.MenuItem("❤️ 健康状态", self.show_health_status),
            self.pystray.MenuItem("🐕 看门狗状态", self.show_watchdog_status),
            ...
            self.pystray.MenuItem("🔄 立即截图", self.take_screenshot_now),
            self.pystray.Menu.SEPARATOR,
            self.pystray.MenuItem("📦 上传队列", self.show_queue_status),
            self.pystray.MenuItem("🧹 清理缓存", self.cleanup_cache),
            self.pystray.MenuItem("🌐 网络诊断", self.network_diagnostic),
            self.pystray.Menu.SEPARATOR,
            # ===== 新增：重新配置菜单项 =====
            self.pystray.MenuItem("✏️ 重新配置", self.reconfigure),
            # ==============================
            self.pystray.MenuItem(
                "⚡ 开机自启",
            ...
            self.pystray.MenuItem("📝 查看日志", self.open_log),
            self.pystray.Menu.SEPARATOR,
    ...
    def reconfigure(self):
        ...
        if result:
            ...
            if success:
                self._show_notification("配置成功", "员工信息已更新")
            else:
                self._show_notification("配置失败", "请检查网络连接后重试")
